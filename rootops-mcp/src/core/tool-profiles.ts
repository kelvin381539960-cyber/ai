...
export const TOOL_PROFILES: Record<ToolProfile, string[]> = {
  ...
  code_edit: ['file.read', 'file.read_many', 'file.search', 'file.hash', 'file.outline', 'patch.dry_run', 'patch.apply', 'patch.verify', 'snapshot.create', 'git.status', 'git.diff', 'build.run', 'local.patch_risk', 'local.context_pack'],
  server_ops: ['ssh.exec', 'ssh.exec_stream', 'ssh.session.open', 'ssh.session.send', 'ssh.session.close', 'rsync.push', 'rsync.pull', 'health.check', 'log.tail'],
  deploy: ['deploy.plan', 'deploy.apply', 'deploy.rollback', 'systemctl', 'log.tail'],
  database: ['database.read', 'database.explain', 'database.write'],
  emergency: ['ssh.exec', 'systemctl', 'log.tail', 'git.rollback', 'deploy.rollback', 'snapshot.restore'],
