...
export function classifyToolRisk(toolName: string): RiskLevel {
  if (/^(file\.read|file\.read_many|file\.list|file\.search|file\.index_|file\.hash|file\.outline|git\.status|git\.diff|git\.log|local\.|audit\.list|audit\.export|policy\.check|task\.scope\.|confirmation\.|safety\.)/.test(toolName)) return 'R0';
  if (/^(snapshot\.create|patch\.dry_run|patch\.plan|patch\.verify|patch\.apply|file\.patch|file\.write|file\.related)$/.test(toolName)) return 'R1';
