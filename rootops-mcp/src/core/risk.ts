...
export function classifyToolRisk(toolName: string): RiskLevel {
  if (/^(file\.read|file\.read_many|file\.list|file\.search|file\.hash|file\.outline|git\.status|git\.diff|git\.log|local\.|audit\.list|audit\.export|policy\.check|task\.scope\.|confirmation\.|safety\.)/.test(toolName)) return 'R0';
