...
- 主表中的 `查询实例` 和 `查询表` 只保留当前账号可直接查询的表。
...
| 40 | `poa_upload_success_frontend` | 前端上传成功，拿到 fileId/fileUrl | POA 文件上传成功后，页面展示文件卡片，Continue 按钮可提交。 | 后端返回 fileId/fileUrl；同时创建 POA 记录、保存 OSS 文件，并更新主申请 POA request_id / 状态。 | ✅ 可直接查 | 业务表可看 `wallet_application.poa_request_id IS NOT NULL` 或 POA 状态字段。 | SG-AIX-AIXPAY-A-OB-MASTER-AWS | aixpay_wallet.wallet_application | `SELECT wa.id, wa.user_id, wa.poa_request_id, wa.poa_status, wa.kyc_status, wa.update_time FROM aixpay_wallet.wallet_application wa WHERE wa.user_id = '${user_id}' AND wa.poa_request_id IS NOT NULL ORDER BY wa.update_time DESC LIMIT 20;` |
