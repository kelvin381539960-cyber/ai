...
- `{{user_id}}`：执行前必须替换为真实用户 ID；不能原样保留变量字符串。
- `${start_time}` / `${end_time}`：查询时间范围，例如 `2026-05-01 00:00:00`。
- 当前 SQL 工具右侧已选择 database/schema 时，查询 SQL 不再写库名前缀，避免执行器解析异常。
...
| 20 | `passport_request_id_saved` | 写入 passport_request_id | 无单独页面动作；这是请求 passport URL 成功后的系统结果。 | 后端保存 passport request_id，并把主申请状态置为处理中。 | ✅ 可直接查 | `aixpay_wallet.wallet_application`：`passport_request_id IS NOT NULL`；同时 `passport_status` 初始化、`kyc_status = 1/PROCESSING`。 | SG-AIX-AIXPAY-A-OB-MASTER-AWS | aixpay_wallet.wallet_application | `SELECT id, user_id, passport_request_id, passport_status, kyc_status, update_time FROM wallet_application WHERE user_id = {{user_id}} AND passport_request_id IS NOT NULL ORDER BY update_time DESC LIMIT 20;` |
...
| 31 | `passport_or_liveness_processing` | 证件/活体处理中 | 轮询页显示处理中，并继续轮询。 | 后端判断 passport 或 liveness 仍在处理中，返回 `PROCESSING`。 | ✅ 可直接查 | 业务表可查 `passport_status=1` 或 `liveness_status=1`。 | SG-AIX-AIXPAY-A-OB-MASTER-AWS | aixpay_wallet.wallet_application | `SELECT id, user_id, passport_status, liveness_status, kyc_status, update_time FROM wallet_application WHERE user_id = {{user_id}} AND (passport_status = 1 OR liveness_status = 1) ORDER BY update_time DESC LIMIT 20;` |
| 32 | `passport_or_liveness_failed` | 证件/活体失败 | 轮询页进入失败状态，展示失败原因和下一步引导。 | 后端判断 passport 或 liveness 失败，返回 `KYC_FAILED`。 | ✅ 可直接查 | `wallet_application.passport_status=2` 或 `liveness_status=2`；如需要子项明细可另查 `kyc_info_record`，但主表 SQL 先保留在 `wallet_application`。 | SG-AIX-AIXPAY-A-OB-MASTER-AWS | aixpay_wallet.wallet_application | `SELECT id, user_id, passport_status, liveness_status, kyc_status, update_time FROM wallet_application WHERE user_id = {{user_id}} AND (passport_status = 2 OR liveness_status = 2) ORDER BY update_time DESC LIMIT 20;` |
...
| 40 | `poa_upload_success_frontend` | 前端上传成功，拿到 fileId/fileUrl | POA 文件上传成功后，页面展示文件卡片，Continue 按钮可提交。 | 后端返回 fileId/fileUrl；同时创建 POA 记录、保存 OSS 文件，并更新主申请 POA request_id / 状态。 | ✅ 可直接查 | 业务表可看 `wallet_application.poa_request_id IS NOT NULL` 或 POA 状态字段。 | SG-AIX-AIXPAY-A-OB-MASTER-AWS | aixpay_wallet.wallet_application | `SELECT id, user_id, poa_request_id, poa_status, kyc_status, update_time FROM wallet_application WHERE user_id = {{user_id}} AND poa_request_id IS NOT NULL ORDER BY update_time DESC LIMIT 20;` |
...
| 43 | `kyc_application_under_review` | KYC 完整提交，进入审核 | POA confirm 成功后，App 跳转 KYC 提交结果页。 | 后端把 KYC 主申请状态更新为 `UNDER_REVIEW`。 | ✅ 可直接查 | `aixpay_wallet.wallet_application`：`kyc_status = 2`；时间字段优先 `update_time`。 | SG-AIX-AIXPAY-A-OB-MASTER-AWS | aixpay_wallet.wallet_application | `SELECT id, user_id, kyc_status, poa_status, update_time, finished_time FROM wallet_application WHERE user_id = {{user_id}} AND kyc_status = 2 ORDER BY update_time DESC LIMIT 20;` |
...
| 45 | `kyc_final_approved` | 最终 KYC 通过 | 无直接前端动作；用户状态由后端审核结果决定。 | 后端获取 DTC 审核结果后，把 `wallet_application.kyc_status` 更新为成功。 | ✅ 可直接查 | `aixpay_wallet.wallet_application`：`kyc_status = 5`；时间字段 `finished_time`。 | SG-AIX-AIXPAY-A-OB-MASTER-AWS | aixpay_wallet.wallet_application | `SELECT id, user_id, kyc_status, passport_status, liveness_status, poa_status, finished_time, update_time FROM wallet_application WHERE user_id = {{user_id}} AND kyc_status = 5 ORDER BY finished_time DESC, update_time DESC LIMIT 20;` |
| 46 | `kyc_final_failed` | 最终 KYC 失败 | 无直接前端动作；用户状态由后端审核结果或异常处理决定。 | 后端获取 DTC 审核结果或处理异常后，把 `wallet_application.kyc_status` 更新为失败。 | ✅ 可直接查 | `aixpay_wallet.wallet_application`：`kyc_status = 3`；时间字段 `finished_time`。 | SG-AIX-AIXPAY-A-OB-MASTER-AWS | aixpay_wallet.wallet_application | `SELECT id, user_id, kyc_status, passport_status, liveness_status, poa_status, finished_time, update_time FROM wallet_application WHERE user_id = {{user_id}} AND kyc_status = 3 ORDER BY finished_time DESC, update_time DESC LIMIT 20;` |
| 47 | `kyc_final_rejected` | 最终 KYC 拒绝 | 无直接前端动作；用户状态由后端审核结果决定。 | 后端获取 DTC 审核结果后，把 `wallet_application.kyc_status` 更新为拒绝。 | ✅ 可直接查 | `aixpay_wallet.wallet_application`：`kyc_status = 4`；时间字段 `finished_time`。 | SG-AIX-AIXPAY-A-OB-MASTER-AWS | aixpay_wallet.wallet_application | `SELECT id, user_id, kyc_status, passport_status, liveness_status, poa_status, finished_time, update_time FROM wallet_application WHERE user_id = {{user_id}} AND kyc_status = 4 ORDER BY finished_time DESC, update_time DESC LIMIT 20;` |

## ✅ 可直接查的行

| 顺序 | 前端动作 | 后端动作 | 能否查 | 查询实例 | 查询表 | 查询 SQL |
|---:|---|---|---|---|---|---|
| 13 | 无单独页面动作；这是用户点击 Continue 后的后端结果。 | 后端保存 `KYC_START` 协议同意记录。 | ✅ 可直接查 | SG-AIX-AIXPAY-A-OB-MASTER-AWS | aixpay_app.user_agreement_consent | `SELECT user_id, location, contract_code, consented_at, create_time, update_time FROM user_agreement_consent WHERE user_id = {{user_id}} AND location = 'KYC_START' ORDER BY consented_at DESC LIMIT 20;` |
| 14 | 无单独页面动作；属于进入/提交 KYC 后的系统处理。 | 后端获取已有 KYC 钱包申请；如果没有则创建新的 `wallet_application`。 | ✅ 可直接查 | SG-AIX-AIXPAY-A-OB-MASTER-AWS | aixpay_wallet.wallet_application | `SELECT id, external_id, user_id, nationality, kyc_status, passport_status, liveness_status, poa_status, create_time, update_time, finished_time FROM wallet_application WHERE user_id = {{user_id}} ORDER BY create_time DESC LIMIT 20;` |
| 15 | 用户在 Start 页提交的居住国家被系统保存。 | 后端把国家写入 `wallet_application.nationality`。 | ✅ 可直接查 | SG-AIX-AIXPAY-A-OB-MASTER-AWS | aixpay_wallet.wallet_application | `SELECT id, user_id, nationality, kyc_status, passport_status, liveness_status, poa_status, create_time, update_time FROM wallet_application WHERE user_id = {{user_id}} ORDER BY update_time DESC LIMIT 20;` |
| 20 | 无单独页面动作；这是请求 passport URL 成功后的系统结果。 | 后端保存 passport request_id，并把主申请状态置为处理中。 | ✅ 可直接查 | SG-AIX-AIXPAY-A-OB-MASTER-AWS | aixpay_wallet.wallet_application | `SELECT id, user_id, passport_request_id, passport_status, kyc_status, update_time FROM wallet_application WHERE user_id = {{user_id}} AND passport_request_id IS NOT NULL ORDER BY update_time DESC LIMIT 20;` |
| 27 | 无单独页面动作；这是请求 liveness URL 成功后的系统结果。 | 后端保存 liveness request_id，并把主申请状态置为处理中。 | ✅ 可直接查 | SG-AIX-AIXPAY-A-OB-MASTER-AWS | aixpay_wallet.wallet_application | `SELECT id, user_id, liveness_request_id, liveness_status, kyc_status, update_time FROM wallet_application WHERE user_id = {{user_id}} AND liveness_request_id IS NOT NULL ORDER BY update_time DESC LIMIT 20;` |
| 31 | 轮询页显示处理中，并继续轮询。 | 后端判断 passport 或 liveness 仍在处理中，返回 `PROCESSING`。 | ✅ 可直接查 | SG-AIX-AIXPAY-A-OB-MASTER-AWS | aixpay_wallet.wallet_application | `SELECT id, user_id, passport_status, liveness_status, kyc_status, update_time FROM wallet_application WHERE user_id = {{user_id}} AND (passport_status = 1 OR liveness_status = 1) ORDER BY update_time DESC LIMIT 20;` |
| 32 | 轮询页进入失败状态，展示失败原因和下一步引导。 | 后端判断 passport 或 liveness 失败，返回 `KYC_FAILED`。 | ✅ 可直接查 | SG-AIX-AIXPAY-A-OB-MASTER-AWS | aixpay_wallet.wallet_application | `SELECT id, user_id, passport_status, liveness_status, kyc_status, update_time FROM wallet_application WHERE user_id = {{user_id}} AND (passport_status = 2 OR liveness_status = 2) ORDER BY update_time DESC LIMIT 20;` |
| 40 | POA 文件上传成功后，页面展示文件卡片，Continue 按钮可提交。 | 后端返回 fileId/fileUrl；同时创建 POA 记录、保存 OSS 文件，并更新主申请 POA request_id / 状态。 | ✅ 可直接查 | SG-AIX-AIXPAY-A-OB-MASTER-AWS | aixpay_wallet.wallet_application | `SELECT id, user_id, poa_request_id, poa_status, kyc_status, update_time FROM wallet_application WHERE user_id = {{user_id}} AND poa_request_id IS NOT NULL ORDER BY update_time DESC LIMIT 20;` |
| 43 | POA confirm 成功后，App 跳转 KYC 提交结果页。 | 后端把 KYC 主申请状态更新为 `UNDER_REVIEW`。 | ✅ 可直接查 | SG-AIX-AIXPAY-A-OB-MASTER-AWS | aixpay_wallet.wallet_application | `SELECT id, user_id, kyc_status, poa_status, update_time, finished_time FROM wallet_application WHERE user_id = {{user_id}} AND kyc_status = 2 ORDER BY update_time DESC LIMIT 20;` |
| 45 | 无直接前端动作；用户状态由后端审核结果决定。 | 后端获取 DTC 审核结果后，把 `wallet_application.kyc_status` 更新为成功。 | ✅ 可直接查 | SG-AIX-AIXPAY-A-OB-MASTER-AWS | aixpay_wallet.wallet_application | `SELECT id, user_id, kyc_status, passport_status, liveness_status, poa_status, finished_time, update_time FROM wallet_application WHERE user_id = {{user_id}} AND kyc_status = 5 ORDER BY finished_time DESC, update_time DESC LIMIT 20;` |
| 46 | 无直接前端动作；用户状态由后端审核结果或异常处理决定。 | 后端获取 DTC 审核结果或处理异常后，把 `wallet_application.kyc_status` 更新为失败。 | ✅ 可直接查 | SG-AIX-AIXPAY-A-OB-MASTER-AWS | aixpay_wallet.wallet_application | `SELECT id, user_id, kyc_status, passport_status, liveness_status, poa_status, finished_time, update_time FROM wallet_application WHERE user_id = {{user_id}} AND kyc_status = 3 ORDER BY finished_time DESC, update_time DESC LIMIT 20;` |
| 47 | 无直接前端动作；用户状态由后端审核结果决定。 | 后端获取 DTC 审核结果后，把 `wallet_application.kyc_status` 更新为拒绝。 | ✅ 可直接查 | SG-AIX-AIXPAY-A-OB-MASTER-AWS | aixpay_wallet.wallet_application | `SELECT id, user_id, kyc_status, passport_status, liveness_status, poa_status, finished_time, update_time FROM wallet_application WHERE user_id = {{user_id}} AND kyc_status = 4 ORDER BY finished_time DESC, update_time DESC LIMIT 20;` |
