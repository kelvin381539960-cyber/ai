...
目标不是 85 分可用方案，而是平均 90+，目标 95+ 的个人高权限 AI 运维/代码操作系统。
...
中等文件：分页读
大文件：chunk 读
日志文件：tail + search window
代码文件：outline + symbol search
二进制/Office/PDF：专用 parser
```
...
4. 实现 File Intelligence Engine，先接 ripgrep + read_lines + read_bytes + read_many。
5. 实现 Patch Transaction Engine，必须支持 expected_hash、dry_run、snapshot、verify、rollback。
6. 接入 Git/status/diff/branch/worktree/commit/push/rollback。
7. 接入 SSH/rsync/session/server group。
8. 补 remote-agent 模式。
9. 补审计日志和任务交接文件自动生成。
...
- 用户偏好：平均评分必须 90+，目标 95+。
- 不要建议低权限阉割版。
- 不要只推荐现成项目原样使用。
- 所有设计都应围绕高权限、低摩擦、可回滚、可审计、可换 AI 工具继续执行。
